...
constexpr static uint32_t READBITS = EVMAP::UNDERLAY_EVENT_READ | ERRBIT | EPOLLHUP;
constexpr static uint32_t WRITEBITS = EVMAP::UNDERLAY_EVENT_WRITE | ERRBIT | EPOLLHUP;
...
class EventEngineEPoll : public MasterEventEngine,
...
public:
    ...
    int ctl(int fd, int op, uint32_t events, int no_log_errno_1=0, int no_log_errno_2=0)
    {
        ...
        {
            ERRNO err;
            if (err.no != no_log_errno_1 && err.no != no_log_errno_2)
            {   // deleting a non-existing fd is considered OK
                auto events = HEX(ev.events);
                auto data = ev.data.ptr;
                LOG_WARN("failed to call epoll_ctl(`, `, `, {`, `})", VALUE(_engine_fd),
                    VALUE(op), VALUE(fd), VALUE(events), VALUE(data), err);
            }
            return -err.no;
        }
        return 0;
    ...
    virtual int add_interest(Event e) override
    {
        ...
        if (op == EPOLL_CTL_MOD && (e.interests & ONE_SHOT) != (entry.interests & ONE_SHOT)) {
            LOG_ERROR_RETURN(EINVAL, -1, "do not support ONE_SHOT on no-oneshot interested fd");
        }
        auto x = entry.interests |= e.interests;
        x &= (EVENT_READ | EVENT_WRITE | EVENT_ERROR);
        // since epoll oneshot shows totally different meanning of ONESHOT in photon
        // all epoll action keeps no oneshot
    ...
    void wait_for_events(uint64_t timeout, const DataCB& datacb, const FDCB& fdcb)
    {
        if (!_events_remain) {
        ...
        while(_events_remain && fdcb())
        {
            ...
            if ((e.events & ERRBIT) && (entry.interests & EVENT_ERROR)) {
                ...
                if (entry.interests & ONE_SHOT) {
                    rm_interest({.fd=(int)e.data.u64, .interests=EVENT_ERROR | ONE_SHOT, .data=nullptr});
                }
                datacb(data);
            }
            if ((e.events & READBITS) && (entry.interests & EVENT_READ)) {
                ...
                if (entry.interests & ONE_SHOT) {
                    rm_interest({.fd=(int)e.data.u64, .interests=EVENT_READ | ONE_SHOT, .data=nullptr});
                }
                datacb(data);
            }
            if ((e.events & WRITEBITS) && (entry.interests & EVENT_WRITE)) {
                ...
                if (entry.interests & ONE_SHOT) {
                    rm_interest({.fd=(int)e.data.u64, .interests=EVENT_WRITE | ONE_SHOT, .data=nullptr});
                }
                datacb(data);
            }
        }
    }
    virtual ssize_t wait_for_events(void** data, size_t count, uint64_t timeout = -1) override
    {
        int ret = get_vcpu()->master_event_engine->wait_for_fd_readable(_engine_fd, timeout);
        if (ret < 0) {
            return ret;
        }
        auto ptr = data;
...
static EventEngineEPoll* new_epoll_engine() {
